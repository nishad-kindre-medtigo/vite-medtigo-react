...
const ClinicalCertificate = () => {
  ...
  // console.log("filteredDepartments for ClinicalCertificate: ", filteredDepartments)

  useEffect(() => {
    const fetchData = async () => {
      try {
        const depts = selectedDepartments.length > 0 ? selectedDepartments : [-1]
        const response = await adminService.getClinicalDonutChartData(depts);
        setTotalCounts(response.data.totalCounts);
      } catch (error) {
        console.error('Error fetching Clinical Certificate Donut Chart Data:', error);
  ...
  return (
    <DetailBox>
      <Box sx={{ mb: 1, display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap' }}>
        <CardTitle title="Clinical Certificate" />
        <GroupFilter
      ...
      <GreyBox sx={{ flexGrow: 1 }}>
        ...
        <AdminDonutChart
          ...
          link="/admin/reports/clinical-certificate"
          handleClick={() => changeGlobalDept(null, filteredDepartments)}
        />
      </GreyBox>
      <ClickHere link="/admin/reports/clinical-certificate" handleClick={() => changeGlobalDept(null, filteredDepartments)} />
    </DetailBox>
  );
};
